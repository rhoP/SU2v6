...
/*!
 * \class CDiscAdjTurbMLVariable
 *  \brief Class for variables of the machine learning adjoint solver.
 * \ingroup Discrete_Adjoint
 * \author Rohit P.
 */
class CDiscAdjTurbMLVariable : public CVariable {
private:
    su2double Sens_MLParam; /* Sensitivity with respect to the machine learning parameter */
    su2double* Sensitivity; /* Vector holding the derivative of target functional with respect to the coordinates at this node*/
    su2double* Solution_Direct;
    su2double* DualTime_Derivative;
    su2double* DualTime_Derivative_n;
...
public:
     ...
     * \param[in] config - Definition of the particular problem.
     */
    CDiscAdjTurbMLVariable(su2double *val_solution, unsigned short val_ndim, unsigned short val_nvar, CConfig *config);
     ...
     * \param[in] val - value of the Sensitivity
     */
    inline void SetSensitivity_MLParam(su2double val) {Sens_MLParam = val;}

    /*!
     * \brief Get the Sensitivity wrt the ML Parameter at the node
     * \return value of the Sensitivity
     */
    inline su2double GetSensitivity_MLParam() {return Sens_MLParam;}
